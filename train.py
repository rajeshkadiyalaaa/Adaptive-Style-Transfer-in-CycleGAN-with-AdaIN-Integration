...
def async_tensorboard_log(writer, logs, step):
    """Log to TensorBoard in a separate thread to avoid blocking training."""
    for log_type, log_data in logs.items():
        if log_type == 'scalars':
            for name, value in log_data.items():
                writer.add_scalar(name, value, step)
        elif log_type == 'image':
            writer.add_image('Images', log_data, step)
...
def main():
    ...
    and optimizer, and trains the model with performance optimizations.
    """
    ...
    dataloader = DataLoader(
        ...
        persistent_workers=opt.num_threads > 0  # Keep workers alive between epochs
    )
    ...
    # Pre-compute LR update frequency
    lr_update_freq = max(1, len(dataloader) // 10)
    
    # Enable automatic mixed precision (AMP) for faster training
    use_amp = torch.cuda.is_available() and not hasattr(opt, 'no_amp')
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):
        ...
        for i, data in enumerate(dataloader):
            ...
            # Perform one step of optimization with AMP
            if use_amp:
                with torch.cuda.amp.autocast():
                    model.optimize_parameters()
                scaler.scale(model.loss_G_A + model.loss_D_A).backward()
            ...
            if total_iterations % opt.print_freq == 0:
                ...
                # Async log to TensorBoard
                logs_to_write = {'scalars': {f'Loss/{name}': val for name, val in losses.items()}}
                thread = Thread(target=async_tensorboard_log, args=(writer, logs_to_write, total_iterations))
                thread.daemon = True
                thread.start()
            
            # Display and save generated images (less frequently to avoid bottleneck)
            if total_iterations % opt.display_freq == 0:
                ...
                # Create a grid of generated images (detached from computation graph)
                with torch.no_grad():
                ...
                # Log images to TensorBoard asynchronously
                logs_to_write = {'image': img_grid}
                thread = Thread(target=async_tensorboard_log, args=(writer, logs_to_write, total_iterations))
                thread.daemon = True
                thread.start()
            ...
            # Update the learning rate (pre-computed frequency)
            if (i + 1) % lr_update_freq == 0:
        ...
        # Save the model checkpoint (consolidated single save)
        if epoch % opt.save_epoch_freq == 0:
            ...
            # Save latest model (single write instead of duplicate)
